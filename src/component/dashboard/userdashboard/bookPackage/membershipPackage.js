...
const MembershipPackageCard = (props) => {
  ...
  const getDoctorSpeciality = async () => {
    let packageId ;
    if(props.location && props.location.state && props.location.state.packageId){
      packageId = props.location.state.packageId;
    }
    try {
      let resp = await httpClient.GET("membership-packages/get-package-details/"+packageId,false,true);
      console.log(resp);
      if (resp.data.status) {
